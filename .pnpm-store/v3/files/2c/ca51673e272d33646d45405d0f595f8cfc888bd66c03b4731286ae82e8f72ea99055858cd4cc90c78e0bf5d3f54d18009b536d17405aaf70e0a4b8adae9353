...
function mergeFields(mergedTypeInfo, typeName, object, fieldNodes, sourceSubschemaOrSourceSubschemas, targetSubschemas, context, info) {
    ...
    delegationMap.forEach((selectionSet, s) => {
        ...
        const valueOrPromise$1 = new valueOrPromise.ValueOrPromise(() => resolver(object, context, info, s, selectionSet)).catch(error => error);
        resultMap.set(valueOrPromise$1, selectionSet);
    });
    return valueOrPromise.ValueOrPromise.all(Array.from(resultMap.keys())).then(results => mergeFields(mergedTypeInfo, typeName, mergeExternalObjects(info.schema, graphql.responsePathAsArray(info.path), object.__typename, object, results, Array.from(resultMap.values())), unproxiableFieldNodes, combineSubschemas(sourceSubschemaOrSourceSubschemas, proxiableSubschemas), nonProxiableSubschemas, context, info)).resolve();
}
