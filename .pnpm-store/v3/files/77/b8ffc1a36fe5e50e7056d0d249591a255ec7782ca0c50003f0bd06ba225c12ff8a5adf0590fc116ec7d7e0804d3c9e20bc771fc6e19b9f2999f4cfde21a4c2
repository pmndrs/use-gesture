...
class WebGPUUniformsGroup extends WebGPUBinding {
  constructor(name) {
    super(name); // the order of uniforms in this array must match the order of uniforms in the shader
    ...
    Object.defineProperty(this, 'isUniformsGroup', {
      value: true
    });
  }
  ...
  getByteLength() {
    ...
    for (let i = 0, l = this.uniforms.length; i < l; i++) {
      const uniform = this.uniforms[i]; // offset within a single chunk in bytes
