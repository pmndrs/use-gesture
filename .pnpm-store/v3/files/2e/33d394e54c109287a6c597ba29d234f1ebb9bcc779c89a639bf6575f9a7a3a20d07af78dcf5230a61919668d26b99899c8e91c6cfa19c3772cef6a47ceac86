...
class WebGPUNodeBuilder extends NodeBuilder {
  ...
  _parseMaterial() {
    const material = this.material; // get shader

    this.nativeShader = ShaderLib.common; // parse inputs
  ...
  getUniformFromNode(node, shaderStage, type) {
    ...
    if (nodeData.uniformGPU === undefined) {
      ...
      if (type === 'texture') {
        ...
        const texture = new WebGPUSampledTexture(uniformNode.name, uniformNode.value); // add first textures in sequence and group for last
