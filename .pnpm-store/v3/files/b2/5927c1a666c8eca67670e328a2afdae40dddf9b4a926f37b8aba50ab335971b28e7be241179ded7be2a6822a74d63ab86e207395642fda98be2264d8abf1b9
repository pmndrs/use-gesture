import { GPUInputStepMode, OneMinusBlendColorFactor, BlendColorFactor, GPUColorWriteFlags, GPUPrimitiveTopology, GPUStencilOperation, GPUVertexFormat, GPUIndexFormat, GPUBlendFactor, GPUBlendOperation, GPUTextureFormat, GPUCompareFunction, GPUFrontFace, GPUCullMode } from './constants.js';
...
class WebGPURenderPipelines {
  ...
  get(object) {
    ...
    if (pipeline === undefined) {
      ...
      const material = object.material; // get shader

      const nodeBuilder = this.nodes.get(material); // shader modules
      ...
      const shaderAttributes = this._parseShaderAttributes(nodeBuilder.vertexShader); // vertex buffers
...
function onMaterialDispose(event) {
  ...
  shaderModules.fragment.delete(nodeBuilder.fragmentShader); // @TODO: still needed remove bindings and pipeline
}
