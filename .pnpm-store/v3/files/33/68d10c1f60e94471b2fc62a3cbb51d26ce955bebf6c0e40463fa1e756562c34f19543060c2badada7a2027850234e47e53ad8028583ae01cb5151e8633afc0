...
function getDirectiveArgumentNodes(data, directive) {
  ...
  keys.forEach(k => {
    ...
    const argNode = {
      ...
      (0, _graphql.astFromValue)(data[k], argumentType) || {
        kind: 'NullValue'
...
function getArgumentsDefinitionNodes(tc, fieldName) {
  ...
  return argNames.map(argName => {
    ...
    return {
      ...
      defaultValue: ac.defaultValue !== undefined && (0, _graphql.astFromValue)(ac.defaultValue, tc.getFieldArgType(fieldName, argName)) || undefined
    };
  }).filter(Boolean);
}
...
function getInputValueDefinitionNodes(tc) {
  ...
  return fieldNames.map(fieldName => {
    ...
    return {
      ...
      description: getDescriptionNode(fc.description),
      directives: getDirectiveNodes(tc.getFieldDirectives(fieldName), tc.schemaComposer),
      defaultValue: fc.defaultValue !== undefined && (0, _graphql.astFromValue)(fc.defaultValue, tc.getFieldType(fieldName)) || undefined
    };
  }).filter(Boolean);
}
