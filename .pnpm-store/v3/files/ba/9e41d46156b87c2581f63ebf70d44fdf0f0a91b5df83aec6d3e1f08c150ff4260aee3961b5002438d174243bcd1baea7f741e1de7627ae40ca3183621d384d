class WebGPUBindings {
  ...
  get(object) {
    ...
    if (data === undefined) {
      ...
      const nodeBuilder = this.nodes.get(material); // each material defines an array of bindings (ubos, textures, samplers etc.)

      const bindings = nodeBuilder.getBindings(); // setup (static) binding layout and (dynamic) binding group
  ...
  update(object, camera) {
    ...
    let needsBindGroupRefresh = false; // iterate over all bindings and check if buffer updates or a new binding group is required
