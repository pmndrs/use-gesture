...
import './WebGPUTextureUtils.js';
import './nodes/WebGPUNodeBuilder.js';
import './nodes/WebGPUNodeUniformsGroup.js';
import './WebGPUUniformsGroup.js';
import './WebGPUBinding.js';
import './nodes/WebGPUNodeUniform.js';
...
import './nodes/ShaderLib.js';
import '../nodes/core/NodeFrame.js';
...
class WebGPURenderer {
  ...
  render(scene, camera) {
    ...
    const passEncoder = cmdEncoder.beginRenderPass(this._renderPassDescriptor); // global rasterization settings for all renderable objects
  ...
  _renderObjects(renderList, camera, passEncoder) {
    ...
    for (let i = 0, il = renderList.length; i < il; i++) {
      const renderItem = renderList[i]; // @TODO: Add support for multiple materials per object. This will require to extract
  ...
  _renderObject(object, passEncoder) {
    const info = this._info; // pipeline
