...
> Changelogs for v10 are now on the [package repo](https://github.com/pmndrs/use-gesture/).
...
1. Fixes a bug with `useTouch` option on non-touch enabled devices that ended up being disabled.
...
1. Should fix [#271](https://github.com/pmndrs/react-use-gesture/issues/271)
...
1. Should fix [#267](https://github.com/pmndrs/react-use-gesture/issues/267).
...
1. Should fix [#260](https://github.com/pmndrs/react-use-gesture/issues/260) (for real).
2. Should fix [#263](https://github.com/pmndrs/react-use-gesture/issues/263).

### Fixed

1. Should fix [#260](https://github.com/pmndrs/react-use-gesture/issues/260).
...
1. Should fix [#260](https://github.com/pmndrs/react-use-gesture/issues/260#event-4321477042).
...
3. Support for pointer lock [#244](https://github.com/pmndrs/react-use-gesture/issues/244).
4. Adds a `locked` flag in the state to indicate whether the document is in lock mode.
5. `useTouch` option for drag in case you want to not lose dragging while the window is scrolling on mobile [#237](https://github.com/pmndrs/react-use-gesture/issues/237).
...
2. Fixed a bug affecting drag when used together with pinch [#234](https://github.com/pmndrs/react-use-gesture/issues/234).
3. Fixes `touches` with the drag gesture [#233](https://github.com/pmndrs/react-use-gesture/issues/233).
4. Normalize wheel event delta values for FireFox [#236](https://github.com/pmndrs/react-use-gesture/issues/236).
...
1. adds origin to GestureEvent (used on Safari Desktop). Fixes [#226](https://github.com/pmndrs/react-use-gesture/issues/226).
...
2. When adding events directly to the dom element using `target` you no longer need to clean the effect yourself.
...
3. Handlers are being fired even if the gesture is not intentional [#3dacf8f](https://github.com/react-spring/react-use-gesture/pull/164/commits/36eec2c4ef241105d0789aaa6ae4c64935d68f19) but only with the `triggerAllEvents` flag set to `true` [#5c77db7](https://github.com/pmndrs/react-use-gesture/commit/5c77db79031800f4610a3f1ec9ee960bdd81e410). This allows manipulation of events at all times. Fixes [#175](https://github.com/react-spring/react-use-gesture/issues/175).
...
1. Only uses webkit gesture events on iOS devices when `target` is specified.
...
4. Added a warning when using trackpad zoom without a `target` or `event.passive` set to true.
...
1. `import { useGesture } from '@use-gesture/react'` should now be functional.
...
3. You can now add a gesture to a dom node or the `window` (useful for scroll) with the `target` config prop. In that case you shouldn’t spread `bind` in a component but use the `useEffect` hook that will take care of adding and removing listeners:

```jsx
const bind = useGesture({ onScroll: () => {...} }, { target: window })
React.useEffect(bind, [bind])
```
...
1. You should now import useGesture as follows: `import { useGesture } from '@use-gesture/react'`
...
2. `target` now accepts refs.
1. Tests are now testing `target`.
