...
&zwnj;<UseGesture pkg="react" /> offers different options to configure the gestures.

Some options are generic to the way <UseGesture pkg="react" /> behaves while some other options can be configured per gesture.
...
Generic options deal with how <UseGesture pkg="react" /> will set event listeners.

| Option                          | Description                                                                                                     |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| [`target`](#target)             | Lets you specify a dom node or React ref you want to attach the gesture to.                                     |
...
| `r3f`                           | Set this option to `true` when using with [`@react-three/fiber`](https://github.com/pmndrs/react-three-fiber)   |
...
| `angleUnit`                                     |   **pinch**    | Defaults to `deg`, unless `r3f` is set to `true` in which case it's `rad`.                                                                                                                           |
...
| [`pointer.touch`](#pointertouch-drag-and-pinch) | **drag,pinch** | If `true`, drag will use touch events on touch-enabled devices. [Read more below](#pointertouch-drag-and-pinch).                                                                                     |
| `pointer.capture`                               |    **drag**    | If `false`, drag will not use `setPointerCapture` and attach `pointerMove` events to the window.                                                                                                     |
| `pointer.lock`                                  |    **drag**    | If `true`, the pointer will enter pointer lock mode when drag starts, and exit pointer lock when drag ends.                                                                                          |
...
&zwnj;<UseGesture pkg="react" /> also supports adding handlers to dom nodes directly (or the <code>window</code> or <code>document</code> objects). In that case, you shouldn't spread the <code>bind()</code> object returned by the
...
`eventOptions.passive` sets whether events are [passive](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener). Note that if you want events not to be passive, you will need to attach events directly to a node using `target` because of [the way React handles events](https://github.com/facebook/react/issues/6436).
...
[^1]: If you're used to <UseGesture pkg="react" />, this was the most common usecase for `memo`.
...
Since v10, `bounds` can also a be a React ref or an HTMLElement, in which case the dragged element will be constrained to the element bounds.
...
When you're interacting with canvas objects, you're dealing with space coordinates that aren't measured in pixels. In that case, you can tell <UseGesture pkg="react" /> to map screen values to the space with a `transform` function.
