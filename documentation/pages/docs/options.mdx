...
| [`bounds`](#bounds)                                | `{top,bottom,left,right}` |  `Infinity`   | Limits the gesture `movement` and `offset` to the specified bounds.             |

## `pinch` specific options

| Option                      |    Type     | Default value | Description                                                          |
| --------------------------- | :---------: | :-----------: | -------------------------------------------------------------------- |
| [`distanceBounds`](#bounds) | `{min,max}` |  `Infinity`   | Limits the distance `movement` and `offset` to the specified bounds. |
| [`angleBounds`](#bounds)    | `{min,max}` |  `Infinity`   | Limits the angle `movement` and `offset` to the specified bounds.    |
...
case, you shouldn't spread the `bind()` object returned by the hooks as a prop, but use the `React.useEffect` hook as
below.
...
function ScrollExample() {
  ...
  const bind = useScroll(({ xy: [, y] }) => set({ width: (y / height) * 100 + '%' }), { domTarget: window })
