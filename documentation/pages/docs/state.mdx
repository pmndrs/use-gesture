...
This is obviously very simple abstract logic, but please go over the [Examples section](examples.mdx) for more compelling use cases. Note that only `drag` and `pinch` gestures are cancellable (calling `cancel` on other gestures won't do anything).
