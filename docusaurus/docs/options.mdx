...
On top of the options for _`xy`_ gestures seen above, _`wheel`_ has the following additional options:

| Option                                  |        Type         | Default value | Description                                                                                                                                                      |
| --------------------------------------- | :-----------------: | :-----------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [_`reversed`_](#reversed-wheel-only)    |      `boolean`      |    `false`    | If `true`, the wheel act as a natural scrolling (usefull for touchscreens and trackpads).                                                                        |
...
Making wheel act as natural scrolling (usefull for touchscreens and trackpads).

<Example id="Reversed" />

```jsx {3-5,7-9}
function ReversedExample() {
  const bind = useWheel(
    ({ down, movement: [mx, my] }) => {
      if (down) alert('down!')
      set({ x: mx, y: my })
