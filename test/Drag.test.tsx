...
describe.each([['attached to component', Interactive, false], ['attached to node', InteractiveDom, true]])(
  ...
  (_testName, C, domTarget): any => {
    ...
    test(`applying a threshold should allow the gesture to start when it's reached`, () => {
      ...
      fireEvent.mouseMove(window, { clientX: 12, clientY: 12, buttons: 1 })
      expect(getByTestId(`${prefix}drag-dragging`)).toHaveTextContent('true')
      expect(getByTestId(`${prefix}drag-offset`)).toHaveTextContent('-18,12')
      expect(getByTestId(`${prefix}drag-movement`)).toHaveTextContent('12,12')
      fireEvent.mouseUp(window)
    })
    ...
    test(`applying an axis SHOULD start the gesture if gesture is detected in the right axis`, () => {
      ...
      expect(getByTestId(`${prefix}drag-dragging`)).toHaveTextContent('true')
      expect(getByTestId(`${prefix}drag-movement`)).toHaveTextContent('20,0')
      fireEvent.mouseUp(window)
    })
    test(`applying a direction lock SHOULD only update the first detected direction`, () => {
      ...
      expect(getByTestId(`${prefix}drag-dragging`)).toHaveTextContent('true')
      expect(getByTestId(`${prefix}drag-movement`)).toHaveTextContent('0,30')
      fireEvent.mouseUp(window)
      expect(getByTestId(`${prefix}drag-dragging`)).toHaveTextContent('false')
      fireEvent.mouseDown(element, { clientX: 0, clientY: 0 })
      fireEvent.mouseMove(window, { clientX: 20, clientY: 10, buttons: 1 })
      ...
      expect(getByTestId(`${prefix}drag-dragging`)).toHaveTextContent('true')
      expect(getByTestId(`${prefix}drag-movement`)).toHaveTextContent('35,0')
      fireEvent.mouseUp(window)
    })
    test(`filtering clicks should fire a click if pointer has moved less than < 3`, () => {
      rerender(<Component gestures={['Drag']} config={{ drag: { filterClicks: true } }} />)
      fireEvent.mouseDown(element, { clientX: 0, clientY: 0 })
