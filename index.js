...
function getEventAttributes(event) {
  ...
  if (event.type === 'scroll') {
    ...
    return { mov_x: scrollX || scrollLeft || 0, mov_y: scrollY || scrollTop || 0, target, type }
  }
  if (event.type === 'wheel') {
  ...
  const { clientX: mov_x, clientY: mov_y, buttons } =
    touches && touches.length > 0 ? touches[0] : changedTouches && changedTouches.length > 0 ? changedTouches[0] : event
  return { mov_x, mov_y, buttons, touches: event.touches && event.touches.length, target, type }
}
...
function handlers(state, props = {}, args) {
  ...
  const injectTouchMouseState = action => eventAttributes => {
    ...
    const stateKey = action === 'onHover' ? (props.onDrag ? 'onDrag' : 'onMove') : action
    ...
    const last = (action === 'onMove' && eventAttributes.debounced) || (action === 'onDrag' && !down)
    const hovering =
      type === mouseEnter || (type === mouseMove && !eventAttributes.debounced) || (state.current[stateKey].hovering && type !== mouseLeave)
    ...
    return {
      ...
      dragging,
      hovering,
      active: (action === 'onDrag' && dragging) || (action === 'onMove' && moving) ? 1 : 0,
      shiftKey,
      first,
      last,
    }
  ...
  const handleGesture = action => event => {
    ...
    const injectSpecificEventState = ['onDrag', 'onMove'].indexOf(stateKey) > -1 ? injectTouchMouseState : injectScrollWheelState
    ...
    state.current[stateKey].temp = props[action](state.current[stateKey]) || state.current[stateKey].temp
  }
  ...
  const cancelDrag = () => {
    ...
    state.current.drag.temp = props.onDrag(state.current.drag) || state.current.drag.temp
  }
